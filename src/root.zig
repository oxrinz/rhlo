...
pub fn execute(program: nodes.RHLOProgram, inputs: []*void, outputs: []*void) void {
    ...
    const h_input = rllvm.types.OpaqueRef{ .ref = core.LLVMConstIntToPtr(core.LLVMConstInt(core.LLVMInt64Type(), @intFromPtr(inputs[0]), 0), core.LLVMPointerType(core.LLVMInt8Type(), 0)) };
    ...
    // gen
    const ptx = @import("backends/cuda/cuda.zig").generatePTX(module, program);
    ...
    const result_ptr = rllvm.types.OpaqueRef{ .ref = core.LLVMConstIntToPtr(core.LLVMConstInt(core.LLVMInt64Type(), @intFromPtr(outputs[0]), 0), core.LLVMPointerType(core.LLVMInt8Type(), 0)) };
...
test "matmul" {
    ...
    var input_buffer = try arena.allocator().alloc(f32, 4);
    input_buffer[0] = 1.0;
    input_buffer[1] = 2.0;
    input_buffer[2] = 3.0;
    input_buffer[3] = 4.0;
    var output_buffer = try arena.allocator().alloc(f32, 4);
    output_buffer[0] = 9;

    var inputs = [_]*void{@ptrCast(input_buffer.ptr)};
    var outputs = [_]*void{@ptrCast(output_buffer.ptr)};
