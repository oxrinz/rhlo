...
pub fn execute(program: nodes.RHLOProgram, input_buffer: buffers.RHLOBuffer, output_buffer: buffers.RHLOBuffer) void {
    // copy data over to device if buffers are passed as host data
...
// this seems weird, but it'll make sense when we consider how the c api will work. or it won't. in that case TODO: make this make sense
